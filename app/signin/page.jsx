...
import { FiMail, FiLock, FiAlertTriangle, FiLoader, FiEye, FiEyeOff } from "react-icons/fi";

// NOTE: Google OAuth still uses the server action approach; email/password uses client-side signIn()
export default function SignInPage() {
    ...
    const handleGoogleSignIn = async () => {
        ...
        await signIn("google", { callbackUrl: "/tips" });
    };
            ...
            <div className="w-full max-w-md glass-panel rounded-[2rem] shadow-2xl overflow-hidden relative z-10 border border-amber-500/20">
                ...
                <div className="pt-12 pb-8 px-8 text-center border-b border-white/5 bg-[#0A0A0A]/50">
                    <Link href="/" className="inline-block mb-6">
                        <div className="w-14 h-14 mx-auto rounded-2xl bg-emerald-500/10 border border-amber-500/30 flex items-center justify-center">
                ...
                <div className="p-8 pt-8">
                    ...
                    <button
                        ...
                        className="w-full flex items-center justify-center gap-3 border border-white/10 bg-white/5 py-4 rounded-full hover:bg-white/10 hover:border-amber-500/30 transition-all mb-8 text-slate-200 font-bold shadow-md disabled:opacity-60"
                    >
                        {googleLoading ? (
                    ...
                    <form onSubmit={handleEmailSignIn} className="flex flex-col gap-5">
                        ...
                        <div>
                            <div className="flex justify-between mb-2 items-end">
                                ...
                                <Link href="/reset-password" className="text-xs font-bold text-amber-400 hover:text-amber-300 transition-colors">
                                    Reset Access?
                                </Link>
                            </div>
                            <div className="relative">
                                ...
                                <input
                                    ...
                                    placeholder="••••••••"
                                    className="w-full pl-11 pr-12 py-4 rounded-xl border border-white/10 focus:outline-none focus:border-amber-500 focus:bg-white/5 bg-[#050505] text-slate-100 placeholder-slate-600 transition-colors"
                                />
                                <button
                                    type="button"
                        ...
                        <button
                            ...
                            className="w-full py-4 rounded-full bg-emerald-600 hover:bg-emerald-500 text-white font-bold text-sm uppercase tracking-wider mt-2 transition-all active:scale-95 shadow-[0_0_20px_rgba(5,150,105,0.4)] disabled:opacity-60 flex items-center justify-center gap-2"
                        >
                            {loading ? <FiLoader className="animate-spin text-xl" /> : "Enter Clinical Portal"}
                        </button>
                    </form>
